...
const navigate = useNavigate()
    ...
    if (Object.values(selectedItems).some((isSelected) => isSelected)) {
      ...
      if (missingFields.length > 0) {
        Modal.confirm({
          ...
          content: (
            <div>
              Vui lòng cập nhật thông tin mua hàng trong hồ sơ trước khi mua sản phẩm: <strong>{missingFields.join(", ")}</strong>
            </div>
          ),
        ...
        const selectedProducts = order.orderItems.filter(item => selectedItems[item.product]);
        navigate('/payment', { state: { total: formattedTotal, selectedProducts } });
  ...
  const calculateTotal = () => {
    const total = order.orderItems.reduce((total, item) => {
      ...
      if (selectedItems[item.product] && !isNaN(itemPrice) && !isNaN(itemAmount)) {
  ...
  console.log('formattedTotal222',formattedTotal)
  return (
      ...
      <div className="container px-5 py-2">
        ...
        <div className="listItemSuperCart">
          ...
          {order?.orderItems?.map((orderItem) => {
            // Lấy ảnh đầu tiên từ mảng ảnh của sản phẩm
                  ...
                  <div className="product-info">
                    <div className="product-info-top d-flex justify-content-between align-items-start">
                      ...
                      <i
                        ...
                        onClick={() => handleRemoveProduct(orderItem.product)}
                      ></i>
                    </div>
                    <div
                      style={{ marginTop: 30 }}
          ...
          <div className="stickyBottomBar d-flex justify-content-between">
            <div className="price-temp">
              <p style={{margin:0}}>Tạm tính</p> <span className="total_price">{formattedTotal}</span>
            </div>
            <button onClick={handleAddCart} className="btn-action">Mua ngay</button>
          </div>
        </div>
      </div>
